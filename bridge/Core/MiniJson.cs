// Adapted from myrcutio/ValheimMCP, MIT. See vendor/ValheimMCP/LICENSE.
using System;
...
namespace ValheimCodexBridge
{
    ...
    public static class MiniJson
    {
        ...
        {
            if (text == null || text.Length > 65536) throw new FormatException("JSON too large");
            ...
            foreach (var c in text) {
                ...
                else if (c == '{' || c == '[') { if (++depth > 24) throw new FormatException("JSON too deep"); }
                else if (c == '}' || c == ']') depth--;
            }
            var i = 0;
            var value = ParseValue(text, ref i);
            SkipWs(text, ref i);
            if (i != text.Length) throw new FormatException("Trailing JSON");
            return value;
        }
        ...
        private static Dictionary<string, object> ParseObject(string s, ref int i)
        {
            ...
            while (true)
            {
                ...
                i++;
                if (obj.ContainsKey(key)) throw new FormatException("Duplicate key");
        ...
        private static string ParseString(string s, ref int i)
        {
            ...
            while (i < s.Length)
            {
                ...
                {
                    if (c < 32) throw new FormatException("Unescaped control character");
                    sb.Append(c);
                }
            }
        ...
        private static double ParseNumber(string s, ref int i)
        {
            ...
            if (double.IsNaN(d) || double.IsInfinity(d)) throw new FormatException("Non-finite number");
            if (!System.Text.RegularExpressions.Regex.IsMatch(token, @"^-?(0|[1-9][0-9]*)(\.[0-9]+)?([eE][+-]?[0-9]+)?$")) throw new FormatException("Invalid number");
            return d;
        }
